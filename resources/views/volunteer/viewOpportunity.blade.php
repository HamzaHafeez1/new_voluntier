            ...
            <div class="row">
                <div class="col-lg-12 animated fadeInRight profile-panel">
                    <div class="col-lg-8 profile-info">
                        <div class="profile-image">
                            <div class="profile-image-hover">
                                <img <?php if($oppr_info->logo_img == NULL){ ?>src="<?=asset('front-end/img/org/001.png') ?>" <?php }else{ ?> src="<?=asset('uploads') ?>/{{$oppr_info->logo_img}} <?php }?>" class="img-circle circle-border m-b-md" alt="profile">
                            </div>
                        </div>
                        ...
                        <div class="profile-text">
                            ...
                            <div class="inner-content">
                                <h2 class="no-margins">{{$oppr_info->opportunity_type}}</h2>
                                <p>({{$oppr_info->start_date}} - {{$oppr_info->end_date}})</p>
                                <h3 class="no-margins">{{$oppr_info->activity}}</h3><br>
                            </div>
                            @if($is_member != 1)
                            <div class="is-joint">
                                <button class="btn btn-primary" id="btn_join"><i class="fa fa-chain"></i> Join</button>
                            </div>
                            <div class="welcome-title" style="color: #ff0000; display: none;">
                                <h3 style="font-size: 20px;">Welcome to our Opportunity!</h3>
                            </div>
                            <div class="joined-title" style="color: #3bb44a; display: none;">
                                <h3>You are a member of our opportunity.</h3>
                            </div>
                            @else
                            <div class="joined-title" style="color: #3bb44a;">
                                <h3>You are a member of our opportunity.</h3>
                            </div>
                            @endif
                        </div>
                    ...
                    <div class="col-lg-4 profile-view-logo">
                        ...
                        <div class="content">
                            <img <?php if($oppr_info->org_logo == NULL){ ?>src="<?=asset('front-end/img/org/001.png') ?>" <?php }else{ ?> src="<?=asset('uploads') ?>/{{$oppr_info->org_logo}} <?php }?>" class="img-circle circle-border m-b-md" alt="profile">
                            <h3><i class="fa fa-star"></i> <a href="Organization-Profile.html"> {{$oppr_info->org_name}} </a> <i class="fa fa-star"></i></h3>
                        </div>
                    </div>
                </div>

                <input type="hidden" id="user_id" value="{{Auth::user()->id}}">
                <input type="hidden" id="oppor_id" value="{{$oppr_info->id}}">
        ...
        @section('script')
            ...
            <script type="text/javascript">
                ...
                $('#btn_join').on('click',function() {
                    var url = API_URL + 'volunteer/opportunity/join';
                    $.ajaxSetup({
                        headers: {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data.result == 'success'){
                                $('.is-joint').hide();
                                $('.joined-title').show();
                                $(".welcome-title").show().delay(5000).fadeOut();
                            }
                        },
                        error: function (data) {
