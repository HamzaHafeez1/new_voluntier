...
//            ');
	...
	public function viewLocationOpportunity(Request $request){
		...
		if(!empty($search_addr['state'])){
            $active_oppr = Opportunity::where(function($query) use ($search_addr) {
                return $query->where('state','like', '%' . $search_addr['state'] . '%')->orwhere('city','like', '%' .$search_addr['city'] . '%');
            });
        }
        else{
            $active_oppr = Opportunity::where('city','like', '%' .$search_addr['city'] . '%');
        }

		$active_oppr = $active_oppr->where('type',1)
            ->where('is_deleted','<>','1')
        ...
        if(!empty($search_addr['state'])){
            $oppr_count_by_id = Opportunity::where(function($query) use ($search_addr) {
                return $query->where('state','like', '%' . $search_addr['state'] . '%')->orwhere('city','like', '%' .$search_addr['city'] . '%');
            });
        }
        else{
            $oppr_count_by_id = Opportunity::where('city','like', '%' .$search_addr['city'] . '%');
        }

		$oppr_count_by_id = $oppr_count_by_id->where('type',1)
            ->where('is_deleted','<>','1')
            ->where('end_date','>=',$today)
		    ->groupBy('category_id')
        ...
        if(!empty($search_addr['state'])){
            $org_count_by_id = Opportunity::where(function($query) use ($search_addr) {
                return $query->where('state','like', '%' . $search_addr['state'] . '%')->orwhere('city','like', '%' .$search_addr['city'] . '%');
            });
        }
        else{
            $org_count_by_id = Opportunity::where('city','like', '%' .$search_addr['city'] . '%');
        }

		$org_count_by_id = $org_count_by_id->where('type',1)->where('is_deleted','<>','1')->where('end_date','>=',$today)
		                              ->groupBy('org_type')->selectRaw('org_type, count(*) as count')->get();
        ...
        if ($location != ''){
            $current_loc = $this->getLocation($location);
        }
        ...
        elseif ($current_loc != 'error'){
            $search_addr['lat'] = $current_loc['lat'];
            $search_addr['lng'] = $current_loc['lng'];
        }
        else{
            $search_addr['lat'] = Auth::user()->lat;
            $search_addr['lng'] = Auth::user()->lng;
	...
	public function getSearchResult(Request $request){
		...
		foreach ($active_oppr as $opp){
            ++$count_opp;
            $new_active_oppr[mb_strtolower(str_replace(" ","",$opp->street_addr1))][] = $opp;
        }
		...
		return Response::json(['search_addr'=>$search_addr,'opprs'=>$new_active_oppr, 'countOpp' => $count_opp]);
	}
...
//		if($oppr->type == Opportunity::REGULAR_OPPORTUNITY){
			...
			if(Opportunity_member::where('oppor_id',$id)->where('user_id',Auth::user()->id)->count()>0)
				$is_member = 1;
			return view('organization.viewOpportunity',['oppr_info'=>$oppr,'is_member'=>$is_member,'page_name'=>'']);
//		}else{
//			return redirect()->to('/');
//		}
	}
