...
class TrackingCtrl extends Controller
{
	...
	public function viewSingleTrackingPage(Request $request){
		...
		$oppr = Opportunity::where('end_date','>=',$today)->where('is_deleted','<>',1)->whereIn('id',$oppr_mem)->get();
		$org_name = User::where('user_role','organization')->where('is_deleted','<>',1)->where('status','<>',1)->get(['id','org_name']);
		return view('volunteer.track',['oppr'=>$oppr,'org_name'=>$org_name,'page_name'=>'']);
	}
	...
	public function joinToOpportunity(Request $request){
		...
		if($is_opp_exist == 1){
			...
			if($is_mem_exist > 0){
				...
				$alert->contents = $joine_dtl->first_name.' '.$joine_dtl->last_name." sent a request to join Opportunity ".$activeoppr;
				$alert->save();
				//send mail
				$opo_admin_email=$opportunity->contact_email ;
				//echo $opo_admin_email;die;
				$comments = $joine_dtl->first_name.' '.$joine_dtl->last_name." sent a request to join Opportunity ".$activeoppr;
				//echo $comments;die;
                if ($opo_admin_email) {
                    Mail::send('emails.requestJoinOpportunity', ['user_type' => 1, 'content' => $comments], function ($message) use ($joine_dtl, $opo_admin_email, $activeoppr) {
                        $message->from('support@myvoluntier.com', 'MyVoluntier.com');
                        $message->to($opo_admin_email);
                        $message->subject($joine_dtl->first_name . ' ' . $joine_dtl->last_name . " sent a request to join Opportunity " . $activeoppr);
                    });
                }
			...
			//add message//
			$activeoppr = Opportunity::find($private_opp->id)->title;
			//echo $activeoppr;die;
			//$activeoppr = "test";
			$joine_dtl = User::select('first_name','last_name')->where('id',$user_id)->first();
			$alert = new Alert;
	...
	public function addHours(Request $request){
        ...
        $dateNew = strtotime ($logged_date);
        $date = strtotime (date('m/d/y'));

        if($date < $dateNew){
            $logged_date = date("m").'/'.date("d") . '/' . date("y");
        }
		...
		if($private_opp_name != ''){
			...
			if($is_edit == 0){
	            ...
	            if($insert_id){
	                $newsfeed  = new NewsfeedModel;
	                $newsfeed->who_joined  = $volunteer_id;
	                $newsfeed->related_id   = Opportunity::find($opp_id)->org_id;
	                $newsfeed->table_name  = 'tracked_hours';
	                $newsfeed->table_id   = $insert_id;
	                $newsfeed->reason   = 'added hours for an opportunity';
	                $newsfeed->created_at    = date('Y-m-d H:i:s');
	                $newsfeed->updated_at    = date('Y-m-d H:i:s');
	                $newsfeed->save();      
	            }
